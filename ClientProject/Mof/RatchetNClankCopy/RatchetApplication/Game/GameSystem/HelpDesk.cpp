...
void ratchet::game::gamesystem::HelpDesk::RegisterQuest(ratchet::game::gamesystem::GameQuest quest) {
    if (_completed) {
        //return;
    } // if
    auto info = Info();
    if (quest.GetType() == ratchet::game::gamesystem::GameQuest::Type::EnemyDestroy) {
        info.text = "敵を倒してアイテムを集めろ！";
        _info.text = info.text;
    } // if
    else if (quest.GetType() == ratchet::game::gamesystem::GameQuest::Type::GoHome) {
        info.text = "船に乗ってゲームクリア！";
    ...
    else if (quest.GetType() == ratchet::game::gamesystem::GameQuest::Type::None) {
        info.text = "";
        _info.text = info.text;
    } // else if
