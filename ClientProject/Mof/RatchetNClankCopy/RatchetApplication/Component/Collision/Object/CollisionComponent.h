#ifndef RATCHET_COLLISION_COMPONENT_H
#define RATCHET_COLLISION_COMPONENT_H
...
public:
    ...
    class CollisionFunc {
        using Func = std::function<bool(const ratchet::CollisionInfo&)>;
    private:
        //! 実行関数
        Func _func;
    public:
        ...
        bool Execute(const ratchet::CollisionInfo& info) const {
            return _func(info);
        }
    };
...
private:
    //! 衝突したオブジェクト
    std::vector<std::weak_ptr<ratchet::CollisionComponent>> _collisioned;
    //! 実行関数
    ...
    /// <param name="in"></param>
    /// <returns></returns>
    bool ExecuteFunction(const std::string& key, const ratchet::CollisionInfo& info, const std::unordered_map<std::string, FuncArray>& in);
protected:
    /// <summary>
    /// 衝突判定
    /// </summary>
...
public:
    ...
    /// <param name=""></param>
    /// <returns></returns>
    virtual std::optional<ratchet::PlaneObject> GetPlaneObject(void) {
        return std::optional<ratchet::PlaneObject>();
    };
    /// <summary>
    /// ゲッター
    /// </summary>
    /// <param name=""></param>
    /// <returns></returns>
    virtual std::optional<ratchet::SightObject> GetSightObject(void) = 0;
    /// <summary>
    /// 判定
    /// </summary>
    /// <param name=""></param>
    ...
    void AddCollisionedObject(const std::shared_ptr<ratchet::CollisionComponent>& ptr);
    /// <summary>
    /// 削除
    /// </summary>
    /// <param name="ptr"></param>
    void RemoveCollisionedObject(const std::shared_ptr<ratchet::CollisionComponent>& ptr);
    /// <summary>
    /// 判定
    /// </summary>
    /// <param name="ptr"></param>
    /// <returns></returns>
    bool ExistCollisionedObject(const std::shared_ptr<ratchet::CollisionComponent>& ptr);
    /// <summary>
    /// 追加
    /// </summary>
    ...
    /// <param name="info"></param>
    /// <returns></returns>
    bool ExecuteEnterFunction(const std::string& key, const ratchet::CollisionInfo& info);
    /// <summary>
    /// 実行
    /// </summary>
    /// <param name="key"></param>
    /// <param name="info"></param>
    /// <returns></returns>
    bool ExecuteStayFunction(const std::string& key, const ratchet::CollisionInfo& info);
    /// <summary>
    /// 実行
    /// </summary>
    /// <param name="key"></param>
    /// <param name="info"></param>
    /// <returns></returns>
    bool ExecuteExitFunction(const std::string& key, const ratchet::CollisionInfo& info);
    /// <summary>
    /// 衝突
    /// </summary>
...
#endif // !RATCHET_COLLISION_COMPONENT_H