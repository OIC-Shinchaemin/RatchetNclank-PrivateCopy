...
MofBool CGameApp::Initialize(void) {
    ...
    theParam.Add(ParamKey::MeshArray      , &(stage.GetMeshArray()));
    theParam.Add(ParamKey::ObjectArray    , &(stage.GetStaticObjectArray()));
    theParam.Add(ParamKey::EnemySpawnArray, &(stage.GetEnemySpawnArray()));
    theParam.Add(ParamKey::GimmickArray   , &(stage.GetGimmickArray()));
    theParam.Add(ParamKey::WoodBoxArray   , &(stage.GetWoodBoxArray()));

    float scene_w = g_pGraphics->GetTargetWidth();
    float scene_h = g_pGraphics->GetTargetHeight();

    main_camera.PerspectiveFov(MOF_ToRadian(60), scene_w / scene_h, 0.1f, 2000.0f);
    main_camera.LookAt(Vector3(100.0f, 40.0f, -200.0f), Vector3(0, 50.5f, 0), Vector3(0, 1, 0));
...
MofBool CGameApp::Update(void) {
    ...
    if (std::optional<RECT> rect = editor::EditorUtilities::ChangeWindowSize()) {
        ...
        main_camera.PerspectiveFov(MOF_ToRadian(60), w / h, 0.1f, 2000.0f);
        main_camera.SetViewPort();
        main_camera.Update();
    }
